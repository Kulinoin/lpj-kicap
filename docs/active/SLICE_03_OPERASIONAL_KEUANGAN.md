...
- User melihat saldo pada detail event yang ditugaskan.
...
- Status event/kegiatan baru selain `draft`, `aktif`, `finish`, `arsipkan`.
...
- Login Admin, buka `/admin`, pastikan menu `Dana Kegiatan` berisi `Dana Masuk Event`, `Mutasi Saldo`, `Saldo User`, `Transaksi Operasional`, dan `Klaim Talangan`.
- Admin catat `Dana Masuk Event`, pastikan `Sumber Dana` berupa pilihan `Lembaga`, `Sponsor`, dan `Dinas`, lalu pastikan total dana event bertambah di data event.
...
- Masuk `Operasional`, buka event aktif demo, dan pastikan yang tampil hanya input operasional/catatan petugas.
- Masuk `Keuangan`, buka event aktif demo, dan pastikan detail event menampilkan saldo pegangan, form pengeluaran, form transfer, form dana talangan, dan riwayat terbaru.
